...
# HM Changes
```{r}
Matrix<-brain.merge@meta.data[brain.merge$Treatment=="Naive",rownames(TaxDef)]
...
CorNaive<-CorAll[rownames(CorAll) %in% c("X0","X1"),]
SigNaive<-SigAll[rownames(CorAll) %in% c("X0","X1"),]

Matrix<-brain.merge@meta.data[brain.merge$Treatment=="FUS",rownames(TaxDef)]
CorAll<-as.data.frame(Hmisc::rcorr(as.matrix(Matrix))$r)
SigAll<-as.data.frame(Hmisc::rcorr(as.matrix(Matrix))$P)
Type<-TaxDef[rownames(CorAll),]
CorFUS<-CorAll[rownames(CorAll) %in% c("X0","X1"),]
SigFUS<-SigAll[rownames(CorAll) %in% c("X0","X1"),]

FullCor<-as.data.frame(rbind(CorNaive,CorFUS))
Clus<-c(rownames(CorNaive),rownames(CorFUS))
Type<-factor(c(rep("Naive",2),rep("FUS-BBBO",2)),levels=c("Naive","FUS-BBBO"))
...
FullSig<-rbind(SigNaive,SigFUS)
FullSig<-FullSig[order(Clus,Type),]
FullSig[is.na(FullSig)]<-''
FullSig<-apply(FullSig,c(1,2),add.significance.stars)

top=columnAnnotation(Cluster=anno_block(gp = gpar(fill = ColorsClusters),labels = c("X0", "X1"),labels_gp = gpar(fontsize=8,col="white"),height = unit(4,"mm")), Treatment=Treatment,col=list(Treatment=c("TRUE"="#FFFFFF","FALSE"="#98BDAA")),show_legend=FALSE, show_annotation_name = FALSE,border=TRUE)

TypeSub<-TaxDef[!(TaxDef$TaxonomyRank2=="Immune cells"),]
FullCor<-as.matrix(FullCor[,colnames(FullCor) %in% rownames(TypeSub)])
TypeSub[c("Non.glutamatergic.neuroblasts","Glutamatergic.neuroblasts"),"TaxonomyRank2"]<-"Neuroblast"
TypeSub[grepl("Telencephalon",TypeSub$TaxonomyRank3),"TaxonomyRank2"]<-"Telencephalon"
...
HMChanges<-Heatmap(t(FullCor),
  ...
  row_names_side = "left",
  column_split = c(rep("X0",2),rep("X1",2)),
...
# Correlation 
```{r}
Cells<-c("X0","X1","X2","X3","X4","X5","X6","Glutamatergic.neuroblasts","Cerebellum.neurons","Enteric.neurons","Peripheral.sensory.peptidergic.neurons","Sympathetic.cholinergic.neurons")
...
Longer<-pivot_longer(Long,cols=c("Glutamatergic.neuroblasts","Cerebellum.neurons","Enteric.neurons","Sympathetic.cholinergic.neurons"))
